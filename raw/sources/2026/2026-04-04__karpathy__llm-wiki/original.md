...
**index.md** is content-oriented. A catalog of everything in the wiki — each page listed with a link, a one-line summary, and optionally metadata. The LLM reads the index first to find relevant pages.

**log.md** is chronological. An append-only record of what happened and when — ingests, queries, lint passes.

## Optional: CLI Tools

At some point you may want to build small tools. A search engine over the wiki pages is the most obvious one. qmd is recommended: a local search engine for markdown files with hybrid BM25/vector search and LLM re-ranking.

## Tips and Tricks

* **Obsidian Web Clipper** for converting web articles to markdown.
* **Download images locally** for LLM access.
* **Obsidian's graph view** to see wiki shape.
